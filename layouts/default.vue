<template>
  <v-app app dark>
    <v-navigation-drawer app v-model="drawer" mobile-break-point="650">
      <v-list subheader>
        <v-subheader>Список друзей</v-subheader>
        ...
        <!-- <v-list-tile v-for="u in users" :key="u.id" @click.prevent> -->
          ...
          <v-list-tile-action>
            ...
            <!-- <v-avatar :color="u.id === user.id ? 'primary' : 'grey'"><span class="white--text text-h6">CJ</span></v-avatar> -->
            <!-- <v-icon :color="u.id === user.id ? 'primary' : 'grey'"><img :src="image.imageURL" alt="avatar"></v-icon> -->
          </v-list-tile-action>
        </v-list-tile>
      </v-list>
    </v-navigation-drawer>
